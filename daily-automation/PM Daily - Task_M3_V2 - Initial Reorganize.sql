...
- Base objetivo actual (USE [BDPRINCIPAL]).
- Permisos para ALTER INDEX y lectura de DMVs.
- Crea dbo.Mantto_Reorganizacion si no existe.
Salidas: Inserta (FechaHora, IndexName, TableName, SchemaName, FragmentationPercentage).
DMVs utilizadas: sys.dm_db_index_physical_stats, sys.indexes, sys.tables.
Seguridad/Impacto: REORGANIZE es menos intrusivo que REBUILD; aún puede consumir IO/CPU.
Uso rápido: EXEC dbo.Tarea_M3_V2;
Notas: Ejecutar después de M2; M4 puede rehacer REBUILD como aseguramiento.
*/
USE [BDPRINCIPAL]
GO

-- Paso 1: Verificar si el SP ya existe antes de crearlo
IF OBJECT_ID('[dbo].[Tarea_M3_V2]', 'P') IS NOT NULL
BEGIN
    DROP PROCEDURE [dbo].[Tarea_M3_V2];
END
GO

-- Paso 1: Verificar si la tabla ya existe antes de crearla
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'dbo' AND TABLE_NAME = 'Mantto_Reorganizacion')
BEGIN
    -- Crear la tabla para almacenar los resultados con marca de tiempo
    CREATE TABLE dbo.Mantto_Reorganizacion (
        FechaHora DATETIME,
...
CREATE PROCEDURE [dbo].[Tarea_M3_V2]
WITH ENCRYPTION
AS
BEGIN
    ...
    WHERE
        ...
        AND ps.avg_fragmentation_in_percent >= 10 AND ps.avg_fragmentation_in_percent < 30

    OPEN IndexCursor
    FETCH NEXT FROM IndexCursor INTO @IndexName, @TableName, @SchemaName, @FragmentationPercentage -- Agregar @FragmentationPercentage aquí
    ...
    BEGIN
        ...
        INSERT INTO dbo.Mantto_Reorganizacion (FechaHora, IndexName, TableName, SchemaName, FragmentationPercentage)
        VALUES (GETDATE(), @IndexName, @TableName, @SchemaName, @FragmentationPercentage);
