...
Notas: Filtra por consultas ejecutadas en la fecha actual (last_execution_time).
*/
USE [BDPRINCIPAL]
GO
...
BEGIN
    ...
    SELECT 
        ...
        s.last_execution_time AS Hora_de_Creacion,
        s.execution_count AS Total_de_Ejecuciones,
        s.total_worker_time / 1000.0 AS Tiempo_Total_de_CPU_ms,
        s.total_elapsed_time / 1000.0 AS Tiempo_Total_Transcurrido_ms,
        s.execution_count * 1.0 / NULLIF(DATEDIFF(SECOND, s.creation_time, GETDATE()), 0) AS Llamadas_por_Segundo
    FROM 
        sys.dm_exec_query_stats AS s
    CROSS APPLY 
        sys.dm_exec_sql_text(s.sql_handle) AS t
    WHERE
        CONVERT(DATE, s.last_execution_time) = CONVERT(DATE, GETDATE());
END
GO