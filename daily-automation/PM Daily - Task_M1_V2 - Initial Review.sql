...
- Base de datos actual (USE [BDPRINCIPAL] por defecto en el repo).
- Permisos de lectura a DMVs y catálogos: sys.dm_db_index_physical_stats, sys.indexes, sys.tables.
- La tabla dbo.Mantto_Revision se crea automáticamente si no existe.
Salidas:
- Inserciones en dbo.Mantto_Revision (FechaHora, IndicesFragmentacionMedia, IndicesFragmentacionAlta, TipoRevision).
DMVs utilizadas: sys.dm_db_index_physical_stats, sys.indexes, sys.tables.
Seguridad/Impacto: Solo lectura de DMVs y una inserción en tabla de auditoría (sin cambios en índices).
Uso rápido: EXEC dbo.Tarea_M1_V2;
Notas: Umbrales: Media 10–30%, Alta >30%. Úsese antes de M2/M3 para medir el estado inicial.
*/
USE [BDPRINCIPAL]
GO

-- Paso 1: Verificar si el SP ya existe antes de crearlo
IF OBJECT_ID('[dbo].[Tarea_M1_V2]', 'P') IS NOT NULL
BEGIN
    DROP PROCEDURE [dbo].[Tarea_M1_V2];
END
GO
...
CREATE PROCEDURE [dbo].[Tarea_M1_V2]
WITH ENCRYPTION
AS
BEGIN
	...
	VALUES (GETDATE(), @IndicesFragmentacionMedia, @IndicesFragmentacionAlta, 'Inicial');
END;
GO